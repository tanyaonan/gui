...
//		label.setVerticalAlignment(SwingConstants.CENTER); // 垂直对其方式
	...
	public void setSelectView(NamedNodeMap arributes, JPanel panel, NodeList childNodes) {
		...
		/**
		 * addItem(String) 添加option
		 * addItemCount() 获取子项数量
		 * getItemAt(Integer) 根据下标获取子项
		 * 
		 * ---下标索引---
		 * ------------
		 * getSelectedIndex()  获取选中项的下标
		 * setSelectedIndex(Integer) 设置选中项
		 * remove(Integer) 移除指定下标项
		 * 
		 * ---内容索引---
		 * ------------
		 * getSelectedItem() 获取选中项内容
		 * setSelectedItem(String) 根据内容设置选中项
		 * remove(String) 移除指定内容的项
		 */
	}
}
